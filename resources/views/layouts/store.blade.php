  ...
  <div  class="jumbotron py-3">
     ...
     <div class="d-flex">
       <div>
          ...
          <tr>
           <td><a href="/part/{{$object->v_id}}/{{$object->f_id}}">{{$object->title}} </a></td>
           <td>{{$object->name}}</td>
          </tr>
...
function YIdtoTotaltime(videoId){
    ...
    ajax.onload = function (e) {
      ...
      // console.log(this.response.items[0].snippet.title);
      Title.innerHTML = this.response.items[0].snippet.title;
      
      // console.log(this.response.items[0].contentDetails.duration);
      // console.log( _convertDuration(this.response.items[0].contentDetails.duration));
    ...
    //   var html = document.getElementById("audio_test");
    //   if (html) html.src = window.URL.createObjectURL(this.response);
    };
}
    ...
    //   Recordingslist.appendChild(hf);
      ...
      function postblob(blob,fname,wavname,mp3name,username,ftitle,comment,vid){
       //XMLHttpRequestによるアップロード処理 //
        ...
        //   post.open("POST", '{{ secure_asset("/request")}}', true);
        //   post.send(formdata);
